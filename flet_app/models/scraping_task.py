"""Modelo de dados para tarefas de scraping - Versão Flet.

Este módulo define a entidade ScrapingTask adaptada para
a aplicação desktop Flet com SQLite.
"""
...
class ScrapingTask(Base):
    ...
    def get_progress_percentage(self) -> int:
        """Calcular percentual de progresso baseado no status.
        ...
        if self.status == 'pending':
            ...
            # Calcular baseado em items_processed (estimativa)
            config = self.get_config()
            max_items = config.get('max_items', 100)
            if max_items > 0:
                return min(int((self.items_processed / max_items) * 100), 95)
            return 50  # Progresso estimado se não tiver max_items
